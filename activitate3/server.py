...
# Dictionary to track file operations
file_operations = {}
...
def log_file_operation(filename, operation):
    """Log file operation"""
    if filename not in file_operations:
        file_operations[filename] = []
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    file_operations[filename].append(f"[{timestamp}] {operation}")
        ...
        while True:
            ...
            try:
                ...
                # File operations
                    ...
                    log_file_operation(filename, f"Created by {current_user}")
                    ...
                    log_file_operation(filename, f"Uploaded by {current_user}")
                    ...
                    if not os.path.exists(old_path):
                        ...
                        try:
                            ...
                            log_file_operation(new_name, f"Renamed from {old_name} by {current_user}")
                            response = {'status': 'success', 'message': f'File renamed from {old_name} to {new_name}'}
                            print(f"✓ File renamed: {old_name} -> {new_name}")
                        except Exception as e:
                            response = {'status': 'error', 'message': f'Error renaming file: {str(e)}'}
                    ...
                    if not os.path.exists(filepath):
                        ...
                        try:
                            with open(filepath, 'r') as f:
                                content = f.read()
                            log_file_operation(filename, f"Read by {current_user}")
                            response = {'status': 'success', 'content': content}
                            print(f"✓ File read: {filename}")
                        except Exception as e:
                            response = {'status': 'error', 'message': f'Error reading file: {str(e)}'}
                    ...
                    if not os.path.exists(filepath):
                        ...
                        try:
                            with open(filepath, 'r') as f:
                                content = f.read()
                            log_file_operation(filename, f"Downloaded by {current_user}")
                            response = {'status': 'success', 'content': content}
                            print(f"✓ File downloaded: {filename}")
                        except Exception as e:
                            response = {'status': 'error', 'message': f'Error downloading file: {str(e)}'}
                    ...
                    if not os.path.exists(filepath):
                        ...
                        try:
                            with open(filepath, 'w') as f:
                                f.write(new_content)
                            log_file_operation(filename, f"Edited by {current_user}")
                            response = {'status': 'success', 'message': f'File {filename} edited'}
                            print(f"✓ File edited: {filename}")
                        except Exception as e:
                            response = {'status': 'error', 'message': f'Error editing file: {str(e)}'}
                    ...
                    if filename not in file_operations:
                        ...
                        history = file_operations[filename]
                    
                    response = {'status': 'success', 'history': history}
                    print(f"✓ File operation history retrieved for: {filename}")
